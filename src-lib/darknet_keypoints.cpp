...
cv::Mat Darknet::Keypoints::annotate(const Predictions & predictions, const Darknet::Skeletons & skeletons, cv::Mat mat)
{
	...
	if (draw_skeleton or draw_keypoints)
	{
		for (size_t skeleton_idx = 0; skeleton_idx < skeletons.size(); skeleton_idx ++)
		{
...
//					cv::rectangle(mat, prediction.rect, colour, 1, line_type);
				...
				else if (draw_keypoints and circle_size > 0)
				{
					// normal keypoint joint
					cv::circle(mat, p, circle_size, colour, cv::FILLED, line_type);
				}
			}

			if (draw_skeleton and line_size > 0)
			{
				for (const auto & [k, v] : SkeletonPoints)
				{
					...
					if (src.x >= 0 and
						...
						dst.y >= 0)
					{
						cv::line(mat, src, dst, colour, line_size, line_type);
					}
				}
					...
					mid_shoulder.y >= 0)
				{
					...
					{
						cv::line(mat, mid_shoulder, points[KP_NOSE], colour, line_size, line_type);
					}
					...
					{
						cv::line(mat, mid_shoulder, points[KP_R_HIP], colour, line_size, line_type);
					}

					if (skeleton[KP_R_HIP] == -1 and skeleton[KP_L_HIP] >= 0)
					{
						cv::line(mat, mid_shoulder, points[KP_L_HIP], colour, line_size, line_type);
					}
#endif
				}
					...
					mid_hip.y >= 0)
				{
					...
					{
						cv::line(mat, mid_hip, points[KP_R_SHOULDER], colour, line_size, line_type);
					}
					if (skeleton[KP_R_SHOULDER] == -1 and skeleton[KP_L_SHOULDER] >= 0)
					{
						cv::line(mat, mid_hip, points[KP_L_SHOULDER], colour, line_size, line_type);
					}
				}
#endif
			}
	...
	if (draw_labels)
	{
		// mostly for DEBUG purpose -- draw the text for each keypoint
